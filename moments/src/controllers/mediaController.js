...
export async function uploadMidia(req, res) {
    ...
    uploadedFrom: (req.headers['user-agent'] || 'unknown').substring(0, 500)
        ...
        console.error(`[UPLOAD_ERROR] User: ${req.user.id}, File: ${arquivo?.originalname}, Error: ${erro.message}`);
        res.status(500).json({
...
async function processarImagemComIA(mediaId, caminhoArquivo, userId) {
    try {
        ...
        const descricao = await gerarDescricaoComGemini(imageBuffer, arquivo.mimetype);
